...
public class SystemActivator implements BundleActivator {
	private final Logger logger = LoggerFactory.getLogger(SystemActivator.class);
	private Runtime runtime;
	private Semaphore lock = new Semaphore(0);

	@Override
	public void start(BundleContext bc) {
		logger.info("Bundle starts: no.ntnu.item.osgi.train.system");
	...
	public void stop(BundleContext bc) {
		if (runtime.getTerminationRequester() == TerminationRequester.FRAMEWORK) {
			...
			logger.info("Received bundle stop command from the OSGi framework: "
				+ "no.ntnu.item.osgi.train.system");
